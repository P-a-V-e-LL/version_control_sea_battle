...
class Player:
    def __init__(self, who="PC", mas=False):
        if mas:
            self.field = mas["field"]
            self.who = mas["who"]
            self.alive = mas["alive"]
            self.possible_targets = mas["p_t"]
            self.suggested_targets = mas["s_t"]
        else:
    ...
    while True:
        while human.alive and comp.alive:               # human player turn
            ...
            while True:                                 # waiting for valid target
                shot = input("Give target coordinate in range A1-J10, Q to lose: ").upper()
                if shot == "Q":
                    human.alive = False
                    break
                if shot in ALL_TARGETS:
                    break
                if shot == "git" or shot == "GIT" or shot == "F" or shot == "B" or shot == "S":
                    break
            if shot != "Q" and shot != "GIT" and shot != "F" and shot != "B" and shot != "S":
                ...
                shot ="A1"
            if shot == "git" or shot == "GIT" or shot == "B" or shot == "F" or shot == "S":
